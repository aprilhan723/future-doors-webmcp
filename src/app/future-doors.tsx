...
type Modal = "bridge" | "tools" | "goal" | "profile" | "why" | null;

const routeNames: Record<RouteId, { label: string; short: string }> = {
  ship: { label: "Ship a product", short: "SHIP" },
  community: { label: "Build in public", short: "CONTRIBUTE" },
  research: { label: "Get mentored", short: "MENTORSHIP" },
};
...
const statusCopy: Record<PathNode["status"], string> = {
  ...
  expired: "CLOSED",
  blocked: "PATH BREAK",
  simulated: "WHAT-IF: DONE",
  strengthened: "PATH STRONGER",
...
function ProfileRail({
  ...
  return (
    <aside className="panel profile-rail" aria-label="Confirmed profile and goal">
      <header className="panel-heading"><span>1</span><div><small>START HERE</small><h2>Your coordinates</h2></div></header>
      ...
      <div className="signal-block"><small>PROVEN STRENGTHS</small><div>{state.profile.strengths.slice(0, 3).map((item) => <span key={item}>✓ {item}</span>)}</div></div>
      <div className="gap-card"><small>GAP TO CLOSE</small><strong>{state.profile.gap}</strong><p>The path should create this—not merely claim it.</p></div>
      <div className="cv-card">
        ...
        <p>Or upload it in chat: your agent can stage facts here for your approval.</p>
      </div>
    </aside>
  );
}
    ...
    <nav className="route-switcher" aria-label="Reachable route options">
      {routes.map((route) => (
        <button key={route.id} className={selected === route.id ? "active" : ""} onClick={() => onSelect(route.id)}>
          ...
          <b>{route.fit}<small>/100 fit</small></b>
        </button>
      ))}
    </nav>
  );
...
function Connector({ node, broken }: { node: PathNode; broken: boolean }) {
  return (
    <div className={`path-connector ${broken ? "broken" : ""}`} aria-hidden="true">
      <span>{node.edgeToNext?.type === "creates" ? "CREATES" : node.edgeToNext?.type === "official" ? "UNLOCKS" : node.edgeToNext?.type === "blocked" ? "MISSING" : "STRENGTHENS"}</span>
      <i><b /></i>
    </div>
  );
}
...
function DecisionDock({ state, route, onTake, onMiss, onRepair, onReset }: { state: FutureDoorsState; route: Route; onTake: () => void; onMiss: () => void; onRepair: () => void; onReset: () => void }) {
  if (route.id !== "ship") return <div className="decision-dock neutral"><div><small>ALTERNATIVE ROUTE</small><strong>{route.summary}</strong></div><button onClick={onReset}>BACK TO BEST ROUTE</button></div>;
  if (state.scenario === "miss") return <div className="decision-dock danger"><div><small>THE CHAIN BROKE</small><strong>No public proof reaches the next door.</strong></div><button className="primary" onClick={onRepair}>PREVIEW SOURCED DETOUR</button><button onClick={onReset}>RESET</button></div>;
  if (state.scenario === "take") return <div className="decision-dock success"><div><small>WHAT-IF COMPLETE</small><strong>Three artifacts now make the next door reachable.</strong></div><button onClick={onReset}>TRY ANOTHER WORLD</button></div>;
  if (state.scenario === "rerouted") return <div className="decision-dock success"><div><small>PATH REPAIRED · HUMAN APPROVED</small><strong>Same useful proof, six weeks later.</strong></div><button onClick={onReset}>RESET DEMO</button></div>;
  return <div className="decision-dock"><div><small>TEST THE PATH</small><strong>See exactly what changes downstream.</strong></div><button className="primary" onClick={onTake}>TAKE THIS DOOR <span>→</span></button><button onClick={onMiss}>MISS IT</button></div>;
}
  ...
  return (
    <section className="panel path-canvas" aria-label="Reachable opportunity path">
      <header className="canvas-heading">
        <div className="panel-heading"><span>2</span><div><small>SEE THE CONSEQUENCE</small><h2>Your reachable path</h2></div></div>
      ...
      <div className="route-summary"><span className="best-dot" /> <strong>{route.summary}</strong><span>Every arrow names what moves forward.</span></div>
      <div className={`path-chain scenario-${state.scenario}`}>
      ...
      <div className="path-legend"><span><i className="solid-line" /> creates a real artifact</span><span><i className="dotted-line" /> strengthens, never guarantees</span><b>Simulation only</b></div>
...
function Inspector({ node, state, onTools }: { node: PathNode; state: FutureDoorsState; onTools: () => void }) {
  ...
  return (
    <aside className="panel inspector" aria-label="Selected step evidence and source">
      <header className="panel-heading"><span>3</span><div><small>CHECK THE BASIS</small><h2>Why this door?</h2></div></header>
      ...
      <div className="control-card"><small>AGENT + YOU</small><div><span className="agent-mark">✦</span><p><b>Agent</b> inspects, compares, simulates, and stages sourced changes.</p></div><div><span className="human-mark">●</span><p><b>You</b> approve profile facts and detours.</p></div><button onClick={onTools}>VIEW {siteToolNames.length} WEBMCP TOOLS</button></div>
...
function WhyModal({ route, profile, onClose }: { route: Route; profile: Profile; onClose: () => void }) {
  return <ModalFrame label="VISIBLE REASONING" title="Why this route ranks here" onClose={onClose} className="why-modal"><div className="reason-row"><span><small>GOAL</small><b>{profile.goal}</b></span><i>+</i><span><small>GAP</small><b>{profile.gap}</b></span><i>+</i><span><small>STRENGTH</small><b>{profile.strengths[0]}</b></span><i>→</i><span className="result"><small>ROUTE</small><b>{routeNames[route.id].label}</b></span></div><div className="why-grid"><div><small>WHY IT FITS</small><p>{route.summary}. It respects {profile.constraints.join(", ").toLowerCase()}.</p></div><div><small>WHAT IT DOES NOT CLAIM</small><p>No score predicts acceptance, selection, a feature, or a job.</p></div></div><footer><span>Fit measures route usefulness for this demo profile.</span><button onClick={onClose}>DONE</button></footer></ModalFrame>;
}
...
export default function FutureDoors() {
  ...
  const reset = useCallback((actor: "you" | "agent" = "you") => { const next = cloneInitialState(); next.profile = stateRef.current.profile; next.activity = [{ id: makeId("activity"), actor, label: "What-if reset", detail: "Best route restored" }, ...stateRef.current.activity].slice(0, 6); stateRef.current = next; setState(next); setModal(null); return summarizeState(next); }, []);
  ...
  const actions = useMemo<FutureDoorsActions>(() => ({
    getPathSnapshot: () => summarizeState(stateRef.current), stageProfileFacts,
    ...
    resetPath: reset,
  }), [commit, reset, selectNode, selectRoute, simulateMiss, simulateTake, stageProfileFacts]);
  ...
  return <main className="app-shell">
    <header className="topbar"><div className="brand"><span className="brand-icon"><i /></span><strong>FUTURE DOORS</strong><small>REACHABLE PATHS, NOT PREDICTIONS</small></div><div className="agent-status"><span className={`capability-dot ${webMcpStatus}`} /><b>{webMcpStatus === "ready" ? "AGENT CONNECTED" : "WEBMCP READY"}</b><span>{siteToolNames.length} shared tools · human approval</span></div><button onClick={() => setModal("tools")}>HOW AGENTS USE THIS</button></header>
    <section className="hero"><div><small>ONE GOAL · MANY RULES · ONE REACHABLE NEXT MOVE</small><h1>Turn official opportunity rules into a path <em>you and your agent can test together.</em></h1></div><p>Your agent brings the research. You confirm the facts. Every door shows what it creates next.</p></section>
